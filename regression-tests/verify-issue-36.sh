...
# - autorevfix: to-tickets 사용 + 기존 출력 경로(issues/issue-<N>-feedback-review-by-<P-version>.md) 보존
# - autotdd: 형제 스킬 예시에 to-tickets, to-spec 존재
# - README: docs/spec-*.md 규약 + /to-spec·/to-tickets 안내 존재
set -uo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
AUTOREVFIX="$REPO_ROOT/.claude/skills/autorevfix/SKILL.md"
...
# ----- 2. autorevfix: to-tickets + 출력 경로 보존 -----
if grep -q 'to-tickets' "$AUTOREVFIX"; then
    pass "autorevfix: to-tickets 사용"
else
    fail "autorevfix: to-tickets 미사용"
fi
if grep -q 'issues/issue-<N>-feedback-review-by-<P-version>\.md' "$AUTOREVFIX"; then
    pass "autorevfix: 과거 호환 출력 경로 보존"
else
    fail "autorevfix: 출력 경로 issues/issue-<N>-feedback-review-by-<P-version>.md 소실"
fi
