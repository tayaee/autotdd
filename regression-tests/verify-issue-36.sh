...
# - autotddreviewfix: to-tickets 사용
# - autotdd: 형제 스킬 예시에 to-tickets, to-spec 존재
# - README: docs/spec-*.md 규약 + /to-spec·/to-tickets 안내 존재
set -uo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
AUTOTDDREVIEW="$REPO_ROOT/.claude/skills/autotddreviewfix/SKILL.md"
...
# ----- 2. autotddreviewfix: to-tickets 사용 -----
if grep -q 'to-tickets' "$AUTOTDDREVIEW"; then
    pass "autotddreviewfix: to-tickets 사용"
else
    fail "autotddreviewfix: to-tickets 미사용"
fi
