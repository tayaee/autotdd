...
5. **Python projects only** (repo has `pyproject.toml`): run `run-ruff`
   ...
   **cost_details 계측 — "before mvp"**: 위 파일 생성 직후,
   `tools/log-cost-<base명>.sh <repo-path> issue-<#N> "before mvp"`를
   호출한다(Windows는 `.bat`/`.ps1` 동일 인자 — pydantic이 필요해 항상
   `uv run`을 거치는 얇은 wrapper이며 셋 다 같은
   `tools/log-cost-<base명>.py`를 감싼다). `<base명>`은 바로 위
   `coders`에 쓴 것과 동일한 키(외부 coder가 지정되지 않은 일반
   실행이면 실행 세션 자신의 base명, 예: `sonnet`). 이 스크립트는
   자기완결적으로 5시간/7일 used_pct를
...
11. Update `issues/issue-#.md`'s `## 구현 결과` section: completion
    ...
    **cost_details 계측 — "after mvp"**: 위 `coders.<base명>.mvp` 채움
    ...
    호출한다(같은 `<base명>`, Step 5의 "before mvp" 호출과 짝을 이룸).
