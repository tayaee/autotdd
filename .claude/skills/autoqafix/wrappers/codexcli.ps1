# codexcli.ps1 — pass-through LLM wrapper for the `codex` CLI (CONTEXT.md
# "LLM 래퍼"). PowerShell port of codexcli.sh.
#
# FLAGS UNVERIFIED — 실 CLI 설치 환경에서 `codex --help`로 확인 후 조정할 것.
# NOTE: this script has not been executed in this environment (no
# PowerShell, no `codex` CLI available under WSL) — verified by existence
# + grep for "codex" only.
#
...
# `codex -p` on stdin; otherwise every argument is passed straight
# through.
