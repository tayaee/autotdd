@echo off
rem ping-codexcli.bat -- thin dispatcher to ping-codexcli.ps1.
rem
rem Batch has no reliable built-in per-process timeout/kill primitive, so
rem the timeout+response-check logic lives in ping-codexcli.ps1 (PowerShell
...
rem or run ping-codexcli.sh under WSL/git-bash instead.
setlocal
set SCRIPT_DIR=%~dp0

where pwsh >nul 2>nul
if %ERRORLEVEL%==0 (
    pwsh -NoProfile -File "%SCRIPT_DIR%ping-codexcli.ps1" %*
    exit /b %ERRORLEVEL%
)

where powershell >nul 2>nul
if %ERRORLEVEL%==0 (
    powershell -NoProfile -File "%SCRIPT_DIR%ping-codexcli.ps1" %*
    exit /b %ERRORLEVEL%
)

echo ERROR: neither pwsh nor powershell found on PATH. 1>&2
echo ping-codexcli.bat needs PowerShell (7+ or Windows PowerShell) to run ping-codexcli.ps1. 1>&2
echo Alternatively, run ping-codexcli.sh under WSL/git-bash. 1>&2
exit /b 1
