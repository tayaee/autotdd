...
기록한다 (`docs/spec/spec-issue-filenames.md` 참조).
...
공개 함수 (라이브러리):
    ...
    build_filename(*, new_n, slug, reviewers, tech_debt)
...
def build_filename(
    *,
    ...
    """파생 이슈 파일명을 조립한다 (v3 규약).

    tech_debt=True  → `issue-<new>-<slug>__tech-debt-by-<r1>-...md` (파킹)
    tech_debt=False → `issue-<new>-<slug>__must-fix-by-<r1>-...md` (pending)

    BY 값은 sort_reviewers 결과를 사용 (`self` 규칙 적용). 원본 이슈 번호는
    파일명에 넣지 않는다(계보는 본문 기록) — v2의 `fixing-<원본>` 세그먼트는
    v3에서 폐지됐다.
    reviewers가 빈 리스트면 ValueError (by 슬롯은 빈 값 불가).
    slug가 빈 문자열이면 ValueError (v3 must-fix/tech-debt는 슬러그 필수).
    """
    if not reviewers:
        raise ValueError("build_filename: reviewers must not be empty")
    if not slug:
        raise ValueError("build_filename: slug must not be empty")
    sorted_rvs = sort_reviewers(reviewers)
    by_value = "-".join(sorted_rvs)
    marker = "tech-debt" if tech_debt else "must-fix"
    return f"issue-{new_n}-{slug}__{marker}-by-{by_value}.md"
