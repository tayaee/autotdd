...
- **Self-Review**: If no reviewers were specified, before launching call
  ...
  `tools/log-cost-<base명>.sh <repo-path> issue-<N> "after review"`를
  호출한다.
...
If not done, first call `tools/log-cost-<base명>.sh <repo-path>
...
9. **cost_details 계측 — "after refix-plan"**: 위 7·8번 write 직후,
   `tools/log-cost-<base명>.sh <repo-path> issue-<N> "after refix-plan"`을
...
- **cost_details 계측 — "before refix"**: 파생 이슈 처리를 시작하기 전,
  ...
  각 파생 이슈 자신의 mvp 비용은 그 이슈 자신의 `agent-stats.json`에
  tdd2가 별도로 기록하므로 여기서 중복 계측하지 않는다.
...
- **cost_details 계측 — "after refix"**: 파생 이슈 처리가 전부 끝난
  직후, `tools/log-cost-<base명>.sh <repo-path> issue-<N> "after refix"`를
