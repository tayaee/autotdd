@echo off
rem ping-qwencli.bat -- thin dispatcher to ping-qwencli.ps1.
rem
rem Batch has no reliable built-in per-process timeout/kill primitive, so
rem the timeout+response-check logic lives in ping-qwencli.ps1 (PowerShell
...
rem or run ping-qwencli.sh under WSL/git-bash instead.
setlocal
set SCRIPT_DIR=%~dp0

where pwsh >nul 2>nul
if %ERRORLEVEL%==0 (
    pwsh -NoProfile -File "%SCRIPT_DIR%ping-qwencli.ps1" %*
    exit /b %ERRORLEVEL%
)

where powershell >nul 2>nul
if %ERRORLEVEL%==0 (
    powershell -NoProfile -File "%SCRIPT_DIR%ping-qwencli.ps1" %*
    exit /b %ERRORLEVEL%
)

echo ERROR: neither pwsh nor powershell found on PATH. 1>&2
echo ping-qwencli.bat needs PowerShell (7+ or Windows PowerShell) to run ping-qwencli.ps1. 1>&2
echo Alternatively, run ping-qwencli.sh under WSL/git-bash. 1>&2
exit /b 1
