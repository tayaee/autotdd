...
```
/autorevfix                (메타 루프 — 위 /autotdd 를 안에서 재호출)
...
이슈당 묶은 조합, `autorevfix`는 그 `autotdd` 자체를 여러 모델이 리뷰·재수정
...
내부적으로 `/autotdd <id> worktree`(A)를 그대로 호출한다. `/autorevfix`(A 최상단)도
같은 지점을 호출한다. 즉 실제 코드 작성은 결국 항상 A의 `/tdd2`+`/acpd` 조합 하나로
수렴하고, B와 `/autorevfix`는 그 위에 "언제·누가·어떤 항목을 무인으로 돌릴지"를
얹은 스케줄러 층일 뿐이다.
