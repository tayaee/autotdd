...
```
/autotddreview             (메타 루프 — 위 /autotdd 를 안에서 재호출)
...
/autotdd                   (오케스트레이터 — 스크립트 없음, 순수 조합)
 이슈 1건을 tdd2 → acpd 순서로 완전히 끝낸 뒤 다음 이슈로. worktree 옵션 가능.
        │
        ├──▶ /tdd2   구현 (red-green-refactor → verify-issue-#.sh → git add 에서 정지)
        │      └─ 의존: ~/.claude/skills/tdd/ (Matt Pocock 원본 스킬)
        │
        └──▶ /acpd   병합·배포 (archive → commit → push → deploy --env dev)
               └─ aacp.sh / aacp.bat / aacp.ps1 + defaults/
```

**한 줄 요약**: `tdd2`(구현)+`acpd`(병합)가 최소 단위 부품, `autotdd`가 이 둘을
이슈당 묶은 조합, `autotddreview`는 그 `autotdd` 자체를 여러 모델이 리뷰·재수정
...
내부적으로 `/autotdd <id> worktree`(A)를 그대로 호출한다. `/autotddreview`(A 최상단)도
같은 지점을 호출한다. 즉 실제 코드 작성은 결국 항상 A의 `/tdd2`+`/acpd` 조합 하나로
수렴하고, B와 `/autotddreview`는 그 위에 "언제·누가·어떤 항목을 무인으로 돌릴지"를
얹은 스케줄러 층일 뿐이다.
