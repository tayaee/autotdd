...
**coder 실수**:
구현자(coder) 평가에서 실수로 세는 것 — 정적분석 도구(ruff·pyright)가 감지한 결함만.
pytest 실패는 어느 단계든 실수가 아니며 기록조차 하지 않는다(TDD red는 의도된 실패이고,
green 과정의 실패도 세지 않기로 확정). 문법 오류(SyntaxError급)는 실수 중에서도 "기초
실수"로 별도 구분해 센다. 측정은 평가받는 모델의 자기 보고가 아니라 로깅 래퍼가 도구
출력에서 기계적으로 뽑는다.
_Avoid_: 버그 수, 테스트 실패 수, 결함(defect 단독)

**defect 밀도**:
coder 실수 합계를 그 이슈에서 추가된 라인 수로 나눈 값 — 구현자 평가의 정본 지표.
분모는 시작HEAD..완료HEAD 범위 전 파일의 added 라인 합(삭제·바이너리 제외, 테스트
포함). 반복 실행으로 중복 감지된 실수는 걸러내지 않고 합산하되 실행 횟수를 항상
병기한다(churn도 실력 신호).
_Avoid_: 에러율, 라인당 버그, 코드 품질 점수
