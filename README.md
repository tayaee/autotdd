...
- **`aacpd`** — archive the issue, `git add -u`, commit code+archive
  ...
  `deploy-to-dev.{sh,ps1,bat}`, run with no arguments, taking priority
  when present; otherwise `deploy.{sh,ps1,bat}`, called as `--env dev`
  — either way a project-provided script this skill never generates).
...
`deploy-to-dev.{sh,ps1,bat}` (priority, no arguments) or
`deploy.{sh,ps1,bat}` (`--env dev`), which `aacp.*` only ever looks for
and calls, never generates.
...
* Add `deploy-to-dev.{bat,ps1,sh}` (no arguments — already dev-specific) or `deploy.{bat,ps1,sh}` (accepting `--env <env>`) to the target repo yourself, to deploy your solution to the `dev` environment. `aacpd` calls `deploy-to-dev.*` with no arguments if present, otherwise `deploy.sh --env dev`; it never creates either file for you.
* Use `/autotdd` or `/autotdd [issue #s]` to repeat `/tdd2` (implementation) and `/aacpd` (`git add -u`, `git commit`, `git push`, then `deploy-to-dev.{bat,ps1,sh}` or `deploy.{bat,ps1,sh}` `--env dev` if the target repo has one).
...
- `deploy-to-dev.{sh,ps1,bat}` (no arguments, priority) or
  `deploy.{sh,ps1,bat} --env <env>` — the deploy entry point, **provided
  by the target project, not generated by `aacpd`**. If neither exists,
  `aacpd` skips the deploy step and tells you to add one.
