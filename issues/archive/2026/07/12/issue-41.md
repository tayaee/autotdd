# issue-41: autotddreviewfix 플래너 이중 게이트 + 파생 이슈 자동 생성·파킹
agent-tier: any
...
autotddreviewfix SKILL.md Step 3·4를 다음으로 재정의한다:
...
- **변경 파일**:
  - `.claude/skills/autotddreviewfix/SKILL.md` (Step 3을 7단계 절차로 재정의: 수집→형식 게이트→분류→must-fix 재검증→파생 이슈 생성→refix-plan→review-stats JSON; Step 4를 pending 파생 이슈 한정 처리 + 파킹 불가침 + JSON 포함 아카이브로 재정의)
