# issue-38: autorevfix → autotddreview 개명 + 위치 인자 문법·인라인 역할 단순화
agent-tier: any
...
- **개명**: `autorevfix` → `autotddreview` ("autotdd 후 review"; 리뷰 결과는
...
- **새 문법**: `autotddreview 1 2 3` (셀프 리뷰), `autotddreview 1 2 3
  minimax deepseek` (외부 리뷰어 2), `autotddreview 1 2 3 worktree minimax`
  (worktree 격리 + 리뷰어 1).
...
1. **개명 (별칭 없음)**: `.claude/skills/autorevfix/` →
   `.claude/skills/autotddreview/`. frontmatter `name`/`description`·트리거
...
8. **회귀 supersede**:
   - `regression-tests/verify-issue-38.sh` 신규 — 새 계약 단언: 새 경로·
     `name: autotddreview`, 옛 플래그 문자열 부재, 위치 인자·worktree·
...
11. **잔여 작업 (리포 밖, 구현 후 별도)**: push 후 npx skills로
    `autotddreview` 전역 설치, `clean-skills.sh` 실행으로 `autorevfix`
    leftover 3겹 제거.

## 승인 기준

- [ ] `.claude/skills/autotddreview/SKILL.md` 존재 + `name: autotddreview`;
...
- **변경 파일**:
  - `.claude/skills/autotddreview/SKILL.md` (신규)
  - `CONTEXT.md` (수정)
  - `cheatsheet.md` (수정)
