...
출발 — 조사 결과 tdd2/autotddreviewfix/aacpd 어디에도 쿼터 계측 훅이 없었다.
...
이 요구사항은 4번(tdd2)·5번(autotddreviewfix) 항목의 "실행 세션 자신의
...
### 5. `autotddreviewfix/SKILL.md` — review/refix-plan/refix 전후 계측, **모델명 파라미터화**
...
### 6. `aacpd/aacp.sh` / `aacp.ps1` — cost_summary 계산 훅 (issue-49와 동일)
...
- [ ] `tdd2/SKILL.md`: Step 5/11에 before/after mvp 계측 지시 —
      **model 식별자가 하드코딩이 아니라 "실행 세션 자신의 값"으로
      명시**돼 있음.
- [ ] `autotddreviewfix/SKILL.md`: Step 2(리뷰어별, self-review는 세션
      자신의 값)/Step 3/Step 4(원본 이슈에 한 쌍만)에 계측 지시 존재,
      전부 model 식별자 하드코딩 없이 파라미터로 지칭.
- [ ] `aacpd/aacp.sh`·`aacp.ps1`: `agent-stats-archive.py` 호출 직전
      `log-cost-summary.py` 존재 확인 후 호출. `aacpd/SKILL.md` 동기화.
...
- `tdd2`/`autotddreviewfix`/`aacpd` SKILL.md에 관련 지시 존재 + model
...
- `skills/autotddreviewfix/SKILL.md` — Step 2(리뷰어별 개별 before/after,
  ...
  금지.
- `skills/aacpd/aacp.sh` / `aacp.ps1` — `agent-stats-archive.py` 호출
  직전, 대상 repo에 `tools/log-cost-summary.py`가 있으면(없으면 스킵)
  먼저 호출하도록 훅 추가.
- `skills/aacpd/SKILL.md` — 위 훅 반영해 문서 갱신.
...
않는다. 그 파일이 없으면(예: 순수 tdd2+aacpd 플로우처럼 review 사이클을
