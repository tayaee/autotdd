...
**구현 완료 일시**: 2026-07-10T18:08:08-0400

**변경 파일**:
- `.claude/skills/autoqafix/usage-claudecli.py` (신규)
- `.claude/skills/autoqafix/usage-minimaxcli.py` (신규)
- `.claude/skills/autoqafix/usage-qwencli.py` (신규)
- `regression-tests/verify-issue-8.sh` (신규)

**계획 대비 편차**: `~/git/harness-project/.local/bin/tmux-usage-bar.py`에
실제 접근 가능해 `claude_usage()`/`minimax_quota()`의 캐시 경로·TTL·API
호출 로직을 그대로 복사(출처 주석 포함)하고, 요구사항 3대로 utilization을
100−utilization으로 잔여율 변환했다. 이 환경에 실제 `mmx` CLI가 설치돼
있어(`~/.local/bin/mmx`), minimax의 "데이터 소스 없음" 회귀 테스트에서 실
네트워크 호출로 새지 않도록 검증 스크립트가 PATH에서 `mmx`가 있는 디렉터리만
선택적으로 제외하도록 만들었다(uv 자체는 유지).

**검증 결과**: `regression-tests/verify-issue-8.sh` 단독 실행 exit 0(승인
기준 전부 PASS, `uv -q run`으로 세 스크립트 모두 실제 기동 확인). 이
환경에 `uv`가 설치돼 있어 PEP-723 유효성도 실제로 검증됨.
`run-regression-tests`로 issue-3~8 전체 실행 결과 PASS=6 FAIL=0. Python
프로젝트가 아니므로(저장소 루트에 `pyproject.toml` 없음) ruff/pyright/pytest
단계는 해당 없음 — 단, usage 스크립트 자체는 PEP-723 인라인 메타데이터로
독립 실행됨을 `uv -q run`으로 직접 확인했다.
