...
서로 다른 두 시점(autotddreviewfix Step 3 / tdd2 Step 5+11 + autotddreviewfix
...
2. 기존 최상위 단일 `date` 필드(review-stats.json, autotddreviewfix Step 3가
...
옮기는 로직이 없다 — autotddreviewfix SKILL.md의 "aacp로 아카이브" 서술은
...
### 4. `autotddreviewfix` SKILL.md 수정 (Step 3, Step 4)

파일: `.claude/skills/autotddreviewfix/SKILL.md`
...
- Step 4의 "`aacp`를 사용해 아카이브"라는 서술을, 요구사항 5의 aacp.sh
  ...
  대해 `aacp`(`.claude/skills/aacpd/aacp.sh`)를 호출하면 `issue-N.md`와
  함께 자동으로 아카이브된다."로 교체(요구사항 5 참조).

### 5. `aacp.sh`/`.ps1` 확장 — TYPE 파일 일괄 아카이브 + agent-stats.json 특별 처리

파일: `.claude/skills/aacpd/aacp.sh`, `.claude/skills/aacpd/aacp.ps1`
...
- 기존 "2. Archive" 스텝(현재 `issue-N.md`만 `git mv`) 바로 다음에 추가:
  ...
  - 매칭되는 `__TYPE-*` 파일이 하나도 없어도(review 사이클 없이 순수
    /tdd2 + /aacpd만 거친 경우) 에러 아님 — 조용히 건너뜀.
...
- [ ] `autotddreviewfix` SKILL.md Step 3: `reviewers`+`derived_by_reviewers`
...
- `tdd2`/`autotddreviewfix` SKILL.md에 `agent-stats.json`, `started`,
...
- `.claude/skills/autotddreviewfix/SKILL.md` — Step 3-7/3-8: agent-stats.json 병합 기록(`reviewers`+`derived_by_reviewers`+`coders.*.review_outcome`, 같은 파일에 대한 한 번의 write), Step 4: done-check·아카이브 목록을 agent-stats.json 하나로, "aacp 호출로 TYPE 파일 일괄 아카이브" 문구로 교체
- `/home/user1/.claude/skills/autotddreviewfix/SKILL.md` — 전역 스킬 동기화
- `.claude/skills/aacpd/SKILL.md` — `__TYPE-*` 산출물 동반 아카이브 문구 추가
- `/home/user1/.claude/skills/aacpd/SKILL.md` — 전역 스킬 동기화
- `.claude/skills/aacpd/aacp.sh` — 2.5단계 신설: 이슈의 살아있는 `__TYPE-*` 파일을 전부 글롭으로 찾아 `git mv`; `agent-stats.json`은 이동 직전 헬퍼 호출로 `archived`/`duration` 스탬프
- `.claude/skills/aacpd/aacp.ps1` — 동일 로직 PowerShell 포트
- `/home/user1/.claude/skills/aacpd/aacp.sh`, `/home/user1/.claude/skills/aacpd/aacp.ps1` — 전역 스킬 동기화
- `.claude/skills/aacpd/defaults/agent-stats-archive.py` — 신규. PEP723 인라인 메타데이터, stdlib만. `started` 기준으로 `archived`(UTC ISO8601)·`duration`(ISO8601 duration) 계산해 같은 파일에 덮어씀. `started` 없으면 에러 종료
- `/home/user1/.claude/skills/aacpd/defaults/agent-stats-archive.py` — 전역 스킬 동기화
...
1. `tools/agent-stats-archive.py`로 계획했던 헬퍼 위치를
   `.claude/skills/aacpd/defaults/agent-stats-archive.py`로 변경. 이유:
   `aacp.sh`는 aacpd 스킬과 함께 다른 target repo에도 배포되는 스크립트인데,
   `tools/`는 이 autotdd 리포 자신의 최상위 디렉터리라 다른 리포에는
   따라가지 않는다 — `tools/`에 두면 다른 프로젝트에서 `/aacpd`를 쓸 때
...
3. `reviewer-scoreboard.py`의 `collect()`가 `reviewers` 키 자체가 없는
   파일(순수 `/tdd2`+`/aacpd`만 거쳐 리뷰 사이클이 없었던 이슈)을 조용히
   건너뛰도록(경고 없이) 구현했다. 이슈 본문의 "실무상 reviewers가 비는
   경우는 없음" 서술은 부정확했다 — 아카이브를 조사해 보니 issue-16/20/21
   등 일부만 리뷰 사이클(autotddreviewfix)을 거쳤고, 대다수 과거 이슈는
   순수 tdd2+aacpd만 거쳐 영구적으로 `reviewers`가 없다. 이를 매번
...
.claude/skills/aacpd/defaults/agent-stats-archive.py` 0 errors.
